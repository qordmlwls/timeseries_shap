...
def train(data, model, n_epochs=400, batch_size=64, verbose=0):
    ...
    try:
        if early_stopping.stopped_epoch !=0:
            stopped_epoch_list.append(early_stopping.stopped_epoch+1)
        
    except:
        if early_stopping.stopped_epoch !=0:
            stopped_epoch_list.append(early_stopping.stopped_epoch+1)
        pass
    return history, stopped_epoch_list
...
def predict(data, model, n_steps=100):
    ...
    # 스케일 된 값에서 실제 값으로 변환
    predicted_subscriber = column_scaler["close"].inverse_transform(prediction)[0][0]

    return predicted_subscriber
...
def create_model(units=50, dropout=0.3, n_steps=100, loss='mae', optimizer='adam', n_layers=4, cell=GRU ,n_features=4):
    # mirrored_strategy2 = tf.distribute.MirroredStrategy()
    # with mirrored_strategy2.scope():
    model = Sequential()
...
def plot_graph(model, data,name):
    ...
    plt.plot(y_test[-200:], c='b', label='actual')
    plt.plot(y_pred[-200:], c='r', label='prediction')
    plt.xlabel("Days")
    plt.ylabel("subscriber")
    plt.legend(["Actual subscriber", "Predicted subscriber"])
    plt.title(name)
    plt.show()
    plt.savefig('../lstm_cook_10month_result/%s구독자예측값실제값.png'%name)